...
function SignalFunnel({ total, proceed, vetoed, skipped }: { total: number; proceed: number; vetoed: number; skipped: number }) {
  const stages = [
    ...
    { label: 'Signal Formed', value: Math.round(total * 0.65), pct: 65, color: '#7c3aed', icon: '📊', desc: 'Pattern matched a strategy' },
    { label: 'AI Approved', value: proceed + vetoed, pct: total > 0 ? Math.round(((proceed + vetoed) / total) * 100) : 35, color: '#2563eb', icon: '🤖', desc: 'Multi-agent review passed' },
