        ...
        for event in events:
            ...
            if event.action in _FULL_CLOSE:
                ...
                try:
                    ...
                    # Map position side (LONG/SHORT) to signal side vocabulary (BUY/SELL)
                    _gr_side = "BUY" if event.side == "LONG" else "SELL"
                    get_graduated_rules_engine().record_outcome(
                        symbol=symbol, regime=_rg_fb, side=_gr_side, won=total_pnl > 0
                    )
                except Exception:
                    pass
