...
class BacktestEngine:
    ...
    def _execute_signal(self, signal: Signal, current_price: float, sim_dt: datetime = None):
        ...
        # Patch open_time and opened_at with simulated time for accurate hold time calculations.
        # Position.__init__ uses datetime.now(UTC) which is wall clock, not sim time.
        # opened_at must also be set so hold_time_rules.should_block_early_exit() fires correctly.
        if sim_dt:
